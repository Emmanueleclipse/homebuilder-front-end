          ...
          for (let i = 0; i < properties.length; i++) {
            ...
            if (activities.length > 0) {
              for (let j = 0; j < activities.length; j++) {
                let from_date = new Date(activities[j]._from + "T00:00:00");

              
                /*let diffTime = Math.abs(
                  timeSetting(from_date) - timeSetting(current_date)
                );*/
                let diffDays = moment(from_date).diff(
                  moment().set({
                    hour: 0,
                    minute: 0,
                    second: 0,
        ...
        {feeds.length > 0 ? (
          feeds.map((item, index) => (
            <div key={index} className="feed-main-card-div my-3 px-3 pt-3 pb-4">
                ...
                <div>
                  ...
                  <div>
                    {item.img.length > 0 && (
                      <img
                        src={
                          `https://homebuilder.herokuapp.com` +
                          item.img[0].attachment
                        }
                        height="100"
                        width="100"
                        alt=""
              ...
              <p className="mt-3 text-14 text-black-50">
                {item.img.attachment}
              </p>
              <div className="btns-div d-sm-flex mt-3">
                ...
                <Link
                  to={"/milestones/" + item.property_id}
